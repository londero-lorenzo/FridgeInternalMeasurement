...
TaskList::TaskList(DHTCollectorList *probeCollectorList): storageDataSDFileSystem() {
...
char *TaskList::createNewTask(int timeDelay) {
    ...
    DEBUG_PRINTLN("...")
    if (TaskList::addNewTask(newTask)) return newTask.getIdentifier();
    DEBUG_PRINTLN("Task not created! (too many tasks)")
    newTask.getIdentifier()[0] = TASK_NOT_CREATED_FULL;
    return newTask.getIdentifier();
}

bool TaskList::addNewTask(Task &createdTask) {
    for (auto &i: TaskList::tasks) {
        Task *probesTimer = &i;
        if (probesTimer->getTimeDelay() == createdTask.getTimeDelay()) return true;
        if (probesTimer->isEmpty()) {
            createdTask.setSDFileSystem(TaskList::storageDataSDFileSystem, TaskList::storageDataRoot);
            i = createdTask;
...
void TaskList::loadFromDisk() {
    SDFile taskRootDirectory = TaskList::storageDataSDFileSystem.open(TaskList::storageDataRoot, FILE_READ);
    // sd/Fridge32/tasks/<-  <IDENTIFIER>.../
    SDFile taskDirectory =taskRootDirectory.openNextFile();
    do {
        Task loadedTask(TaskList::storageDataSDFileSystem, taskDirectory.path());
        if (loadedTask.isActivated()) {
            loadedTask.setDHTCollectors(TaskList::probeCollectorList);

        }
        taskDirectory = taskDirectory.openNextFile();
    }while (!taskDirectory);
}
...
void TaskList::setSDFileSystem(SDMaster &sdMaster, const char* root) {
    TaskList::storageDataSDFileSystem = sdMaster;
