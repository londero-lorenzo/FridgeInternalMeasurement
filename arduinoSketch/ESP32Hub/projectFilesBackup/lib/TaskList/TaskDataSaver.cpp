...
TaskDataSaver::TaskDataSaver(const char *taskIdentifier): fileSystem(SDFS(nullptr)) {
...
void TaskDataSaver::save(DHTData *data) {
    ...
    if (TaskDataSaver::taskDataFile.available()){
        ...
        //TaskDataSaver::taskDataFile.print(rawDataToSave);
    }
...
void TaskDataSaver::setSDFileSystem(SDFS &newFS, const String& root) {
    TaskDataSaver::fileSystem = newFS;
    ...
    if (TaskDataSaver::fileSystem.mkdir((TaskDataSaver::dataStorageDirectory).c_str())) {
        ...
        TaskDataSaver::taskDataFile = FileHandler(&(TaskDataSaver::fileSystem), root, String(DATA_FILE));
    }
